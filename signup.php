...
/*    if (!preg_match("/^[A-Z]{1}[12ABCD]{1}[0-9]{8}$/", $id)){
        ...
        header("refresh:1;url=singup.html") ;
    }
*/
    // 切開字串
    ...
    $sql = "select * from user_list where user = '$user' " ;
    $result = mysqli_query($connect , $sql);

    $num = mysqli_num_rows($result);


    if($user && $password && $ID && $phone && $email)
    {
        if(!checkId($ID))
        {
            echo '身分證號碼錯誤' ;
        ...
        {
            $sql = "insert into user_list(user,password,ID,phone,email) values ('$user','$password','$ID','$phone','$email')" ;
